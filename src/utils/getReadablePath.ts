...
export function PathUtils(path){
  ...
  function getParentUrl(path){
    return (path == 'root') ? 
    `${apiConfig.driveApi}/${path}` : 
    `${apiConfig.driveApi}/root:/${pathSegments.join('/')}`;
  }
  
  function getChildUrl(path, parentItemId){
    return (path == 'root') ?
    `${apiConfig.driveApi}/${path}/children` :
    `${apiConfig.driveApi}/items/${parentItemId}/children`;
  }
...
PathUtils.readQuery = (path:string | string[]) => {
  // the "path" query parameter could evaluate to an array
  // - we expect: ~/?path=sub,folder ("sub,folder")
  // - but this is also valid: ~/?path=sub&path=folder (which evaluates to  ["sub", "folder"]
  
  // always return the query parameter as
  // comma-separated values regardless
